...
return temp_user
  ...
  async function handleSignup() {
    ...
    let body = JSON.stringify({email:signupEmail,password:signupPassword,display_name:displayName}) 
    console.log(body)
      let res = await fetch(url ,{method:'POST',body:body,    headers: {
...
{#if !isSignup}
  ...
  <form>
    ...
    <button on:click="{()=>{isSignup = true}}" style="margin-top:10px" class="button">New? Signup</button>
{/if}
{#if isSignup}
  <h1>Signup</h1>
  <form>
    ...
    <button on:click="{()=>{isSignup = false}}" style="margin-top:10px" class="button">To Signin</button>
{/if}
